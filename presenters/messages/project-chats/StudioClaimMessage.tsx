...
const StudioClaimMessage = memo(({ onCounter, onConfirm, item, last_negotiation_id, last_negotiation_status }: IProps) => {
	...
	const handleCounter = useCallback(() => {
		...
		SheetManager.show('Drawer', {
			payload: {
				sheet: SheetType.StudioReviseClaim,
				data: {
					sender_name,
					conversation_id,
	...
	const handleConfirm = useCallback(async () => {
		...
		try {
			const response = await server.post(CONSTANTS.endpoints.studio_accept_claim(item?.content?.negotiation?.id));
	...
	return (
		<>
			...
			<View style={styles.body}>
				<FieldItem orientation="row" label="Amount" value={item?.content?.negotiation?.amount ? `${item?.content?.negotiation?.amount}` : '-'} style={styles.field} />
				...
				<FieldItem orientation="row" label="Payment Terms:" style={styles.field} value={`${item?.content?.negotiation?.payment_terms} Days`} />
			</View>
			{!sentByMe && (
...
export default StudioClaimMessage;
