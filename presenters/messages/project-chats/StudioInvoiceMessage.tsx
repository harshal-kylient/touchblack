...
export default function StudioInvoiceMessage({ item, project_id }) {
	return (
		<View style={{ gap: 1 }}>
			<InvoiceComponent project_id={project_id} item={item} />
			<TouchblackInvoiceComponent item={item} />
		</View>
	);
}

function TouchblackInvoiceComponent({ item }) {
	...
	return (
		<View style={styles.container}>
			...
			<View style={styles.invoiceDetail}>
				...
				<Pressable style={{ paddingLeft: theme.padding.base }} onPress={() => Linking.openURL(`https://support.touch.black/touchblack-invoices/${touchblackInvoiceId}?token=${authToken}`)}>
					<Download size="24" />
				</Pressable>
			</View>
		</View>
...
const InvoiceComponent: React.FC<InvoiceComponentProps> = ({ item, project_id }) => {
	const { styles, theme } = useStyles(stylesheet2);
	const { authToken } = useAuth();
	...
	return (
		<View style={styles.container}>
			...
			<View style={styles.invoiceDetail}>
				...
				<Pressable style={{ paddingLeft: theme.padding.base }} onPress={() => Linking.openURL(`https://support.touch.black/invoices/${invoiceId}?token=${authToken}`)}>
					<Download size="24" />
				</Pressable>
			</View>
		</View>
