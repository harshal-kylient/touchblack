...
export default function ProducerCalendarView() {
	...
	const remainingHeight = screenHeight - (calendarHeaderHeight + legendHeight + tabNavigatorHeight);
	...
	const { data: producer_projects } = useGetProducerCalendarList(formattedMonth);
	const markedDates = producer_projects?.reduce((acc, { date, completed, live }) => {
		acc[date] = {
		...
		() => ({
			...
			'stylesheet.calendar.main': {
				...
				dayContainer: {
					...
					height: (UnistylesRuntime.screen.height - (Platform.OS === 'android' ? 37 : 40) - (Platform.OS === 'android' ? 45 : 90) - (Platform.OS === 'android' ? 2 : 3) * 43) / 6,
				},
				monthView: {
	...
	const handleDayPress = (day: DateData) => {
		...
		if (counts?.blackoutCount || counts?.completedCount || counts?.confirmedCount || counts?.enquiryCount || counts?.liveCount || counts?.totalCount) {
			navigation.navigate('ProducerProjectsPerDayView', { day });
		}
	};
...
const stylesheet = createStyleSheet(theme => ({
	...
	dot: (status: IStatus) => ({
		...
		height: 8,
		borderRadius: 50,
		backgroundColor: status === EnumStatus.Confirmed ? theme.colors.success : status === EnumStatus.Enquiry ? theme.colors.primary : status === EnumStatus.Blocked ? '#50483b' : status === EnumProducerStatus.Completed ? theme.colors.verifiedBlue : status === EnumProducerStatus.Live ? theme.colors.success : theme.colors.muted,
	}),
	legend: {
		flexDirection: 'row',
