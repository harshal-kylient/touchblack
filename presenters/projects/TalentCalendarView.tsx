...
export default function TalentCalendarView() {
	...
	const { data: talent_projects, refetch } = useGetTalentCalendarList(userID!, formattedMonth);
		...
		() => ({
			...
			'stylesheet.calendar.main': {
				...
				dayContainer: {
					...
					height: (UnistylesRuntime.screen.height - (Platform.OS === 'android' ? 40 : 50) - (Platform.OS === 'android' ? 45 : 90) - (Platform.OS === 'android' ? 3 : 4) * 43) / 6,
				},
				monthView: {
	...
	const handleDayPress = (day: DateData) => {
		...
		if (counts?.blackoutCount || counts?.completedCount || counts?.confirmedCount || counts?.enquiryCount || counts?.liveCount || counts?.totalCount || counts?.interestedCount) {
			navigation.navigate('TalentProjectsPerDayView', { day });
		}
	};
	...
	return (
		<ScrollView style={styles.container}>
			...
			<FloatingButton onPress={() => navigation.navigate('BlackoutDates')} />
		</ScrollView>
	);
}
...
const stylesheet = createStyleSheet(theme => ({
	...
	dot: (status: IStatus) => ({
		...
		height: 8,
		borderRadius: 50,
		backgroundColor: status === 'Tentative' ? theme.colors.primary : status === 'Confirmed' ? '#F56446' : status === 'Enquiry' ? theme.colors.success : status === 'Blackout' ? theme.colors.muted : theme.colors.muted,
	}),
	legend: {
		flexDirection: 'row',
