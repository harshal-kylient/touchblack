...
export const ServiceDaysTable: React.FC<ServiceDaysTableProps> = ({ data }) => {
...
export const PriceListTable: React.FC<PriceListTableProps> = ({ data, prices, setPrices, total, title = 'Price List' }) => {
	const { styles, theme } = useStyles(stylesheet);
		...
		<View style={styles.tableContainer(false)}>
			...
			{data?.map((item, index) => (
				<View style={[styles.tableRow(), { borderRightWidth: theme.borderWidth.slim }]} key={`row-${index}`}>
					...
					<View style={styles.tableContent}>
						<TextInput value={String(prices?.[item?.name] || '')} onChangeText={value => setPrices(prev => ({ ...prev, [item?.name]: value }))} style={styles.input} />
					</View>
				</View>
			))}
