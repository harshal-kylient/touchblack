...
export default function ReviseClaim({ onSuccess, comments, cancellation_charges, payment_terms, gst_applicable, conversation_id, negotiation_id, sender_name, amount }: IProps) {
	...
	const onSubmit = async (data: ReviseClaimFormValues) => {
		try {
			const response = await server.post(CONSTANTS.endpoints.revise_claim(negotiation_id), data);
