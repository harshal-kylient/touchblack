...
const RADIO_OPTIONS = [
	{ value: 'appreciate', label: 'Appreciate it, but not the right fit.' },
	{ value: 'honored', label: 'Honored, but other commitments.' },
	{ value: 'grateful', label: 'Grateful, but personal reasons.' },
	{ value: 'other', label: 'Other' },
] as const;

interface IProps {
	onSuccess?: () => void;
	invitation_id: UniqueId;
	conversation_id: UniqueId;
}

const OptOut = memo(({ onSuccess, invitation_id, conversation_id }: IProps) => {
	...
	const onSubmit = useCallback(
		async (data: OptOutFormValues) => {
			try {
				const reasonValue = data.reason === RADIO_OPTIONS.find(it => it.value === 'other')?.value ? data.customReason : RADIO_OPTIONS.find(it => it.value === data.reason)?.label;
				const response = await server.postForm(CONSTANTS.endpoints.opt_out(invitation_id, conversation_id, EnumStatus.Opted_out), { reason: reasonValue });
		...
		[form, onSuccess, conversation_id, invitation_id, navigation],
	);
