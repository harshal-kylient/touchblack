...
export default function CounterOffer({ onSuccess, comments, cancellation_charges, payment_terms, gst_applicable, conversation_id, negotiation_id, sender_name, amount }: IProps) {
	...
	const onSubmit = async (data: NegotiationFormValues) => {
		try {
			const response = await server.post(CONSTANTS.endpoints.counter_offer(negotiation_id, data.amount), data);
	...
	function calculateTotalAmount() {
		return formatAmount(form.watch('amount'), form.watch('gst_applicable'));
	}
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem style={styles.formItem}>
								<FormLabel style={styles.formLabel}>Amount (exclusive of GST)</FormLabel>
								<FormControl>
									<TextInput
