...
export default function useGetAllMessages(converstation_id: UniqueId, project_id?: UniqueId) {
	const { loginType, userId, producerId } = useAuth();
	const { studioFloor } = useStudioContext();
	const { selectedTalent } = useTalentContext();
	const managerTalentId = selectedTalent?.talent?.user_id;
	const owner_id = loginType === 'producer' ? producerId : loginType === 'studio' ? studioFloor?.id : loginType === 'manager' ? managerTalentId : userId;
