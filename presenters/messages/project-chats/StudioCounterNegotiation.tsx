...
import { View, Pressable, ScrollView, Platform, SafeAreaView, Keyboard } from 'react-native';
...
import { CounterServiceDaysTable, CounterPriceListTable } from '@presenters/studio/components/CounterPricingInputTable';

interface IProps {
	route: any;
}

export default function StudioCounterNegotiation({ route }: IProps) {
	...
	const advance_amount = String(route.params?.advance_amount ?? '');
	const full_advance = route.params?.full_advance;
	const amount = route.params?.amount;
	const payment_terms = String(route.params?.payment_terms ?? '');
	const cancellation_charges = route.params?.cancellation_charges;
	const comments = route.params?.comments;
	const gst_applicable = route.params?.gst_applicable;
	const status = route.params?.status;

	const negotiation_id = route.params?.negotiation_id;
	...
	const terms_and_conditions_url = route.params?.terms_and_conditions_url;
	const [prices, setPrices] = useState(services || {});

	const total = prices
		? Object.values(prices).reduce((acc, curr) => {
				const numValue = Number(curr?.total);
	...
	const onSubmit = useCallback(
		async (data: NegotiationFormValues) => {
			try {
				const response = await server.post(CONSTANTS.endpoints.studio_counter_offer(negotiation_id), {
					...data,
		...
		<SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.backgroundDarkBlack }}>
			...
			<ScrollView style={{ maxHeight: '80%', marginBottom: bottomMargin }}>
				<View style={styles.tableView}>
					<CounterServiceDaysTable data={SERVICE_DAYS_DATA} />
					<CounterPriceListTable total={total} prices={prices} setPrices={setPrices} title="Price List" />
				</View>
			...
			<View style={styles.bottomView}>
				<View style={styles.totalPriceContainer}>
					...
					<Text size="primarySm" style={styles.totalPriceText} weight="bold" color="regular">
						₹ {form.watch('gst_applicable') ? Number(total) * 1.18 : total}/-
					</Text>
					<Text size="primarySm" style={styles.totalPriceText} color="muted">
						{' '}
