		...
		<View style={styles.talentListContainer}>
			...
			<View style={styles.flashListWrapper}>
				<FlashList
					...
					renderItem={({ item }) => <UserItem name={(item.first_name || '') + ' ' + (item?.last_name || '')} id={item.id} verified={item.verified} profession={item.profession_type} onPress={() => navigation.navigate('TalentProfile', { id: item?.id })} image={createAbsoluteImageUri(item.profile_picture_url)} />}
