...
export default function StudioConversationItem({ item, index, studioChat }: IProps) {
	...
	const handleMessageItemPress = (item: IMessage) => {
		if (item?.conversation_type === 'producer_studio_floor') {
			navigation.navigate('StudioConversation', { project_name: item?.project_name, id: item?.id, party1Id: item?.party1_id, party2Id: item?.party2_id, project_id: item?.project_id, receiver_id, name: item?.reciever_name, picture: item.reciever_profile_picture, owner_profile_picture: item.reciever_profile_picture, receiver_type: loginType === 'producer' ? 'User' : item.party2_type });
			return;
		} else if (item?.conversation_type === 'project') {
			navigation.navigate('ProjectConversation', { id: item?.id, party1Id: item?.party1_id, party2Id: item?.party2_id, project_id: item?.project_id, receiver_id, name: item?.reciever_name, picture: item.reciever_profile_picture, owner_profile_picture: item.reciever_profile_picture, receiver_type: loginType === 'producer' ? 'User' : item.party2_type });
			return;
		} else navigation.navigate('Conversation', { id: item.id, party1Id: item?.party1_id, party2Id: item?.party2_id, receiver_id, name: item.reciever_name, picture: item.reciever_profile_picture, receiver_type: loginType === 'producer' ? 'User' : item.party2_type });
	};
