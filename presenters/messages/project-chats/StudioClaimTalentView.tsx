...
const StudioClaim = memo(({ onCounter, onConfirm, item, enabled, conversation_id, sender_name }: IProps) => {
	...
	const extraData = JSON.parse(item?.content?.negotiation?.extra_data);
	const { setup_days, shoot_days, dismantle_days } = useStudioConversationContext();
	...
	return (
		<>
			...
			<View style={styles.body}>
				...
				<FieldItem orientation="row" label="Payment Terms:" style={styles.field} value={item?.content?.negotiation?.payment_terms ? `${item?.content?.negotiation?.payment_terms} Days` : '-'} />
			</View>
				...
				<>
					...
					<View style={styles.pricingListContainer}>
						<View style={styles.tableContainer}>
							...
							<View style={{ flexDirection: 'row' }}>
								<View style={{ minWidth: '33.33%', maxWidth: '33.33%', justifyContent: 'center', alignItems: 'center', borderRightWidth: theme.borderWidth.slim, borderColor: theme.colors.borderGray }}>
