...
import StudioConversationRenderItem from './StudioConversationRenderItem';
import { useStudioContext } from '@presenters/studio/StudioContext';
import EnumStudioStatus from '@models/enums/EnumStudioStatus';

/*
 * params: route: {
 *		project_id: UniqueId, Project ID
 *		id: UniqueId, Conversation ID
 * }
 *
 */
export default function StudioConversation({ route }) {
	// Route params
	const project_id = route.params?.project_id;
	const id = route.params?.id; // conversation id

	const { userId, producerId, loginType, studioId, permissions } = useAuth();
	const { studioFloor } = useStudioContext();
	const myId = useMemo(() => (loginType === 'studio' ? studioFloor?.id : loginType === 'producer' ? producerId : userId), [loginType, producerId, userId, studioId, studioFloor]);
	const { styles, theme } = useStyles(stylesheet);
	const navigation = useNavigation();
	const [loading, setLoading] = useState(false);

	const { data: response, refetch, isLoading, hasNextPage, fetchNextPage } = useGetAllMessages(id, project_id);
	...
	const reciever_profile_picture = response?.metadata?.reciever_data?.reciever_profile_picture;

	const project_name = response?.metadata?.project_name;
	const receiver_id = response?.metadata?.reciever_data?.reciever_id;
	const name = response?.metadata?.reciever_data?.reciever_name;
	const party1Id = loginType === 'studio' ? studioFloor?.id : response?.metadata?.reciever_data?.reciever_id; // Party 1 is always other than producer
	const party2Id = loginType === 'producer' ? producerId : response?.metadata?.reciever_data?.reciever_id; // Party 2 is always the producer
	...
	const active = (loginType === 'studio' || loginType === 'producer' ? permissions?.includes('Messages::Edit') : true) && status !== EnumStudioStatus['Not available'];
	...
	const sendMessage = useCallback(
		async (conversation_id: UniqueId) => {
			...
			if (response2.data?.success) {
				setInputText('');
				Keyboard.dismiss();
				await queryClient.invalidateQueries(['useGetAllMessages', conversation_id, myId!, studioFloor, project_id]);
	...
	const handleProfileNavigation = useCallback(() => {
		if (loginType === 'producer') {
			navigation.navigate('StudioDetails', { id: receiver_id });
		} else if (loginType === 'studio') {
			navigation.navigate('ProducerProfile', { id: party2Id });
		}
	...
	return (
		<SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.backgroundDarkBlack }}>
				...
				<Pressable onPress={handleProfileNavigation} style={{ backgroundColor: theme.colors.backgroundDarkBlack, zIndex: 99, flex: 1, justifyContent: 'center' }}>
					<View style={{ flexDirection: 'column', backgroundColor: theme.colors.backgroundDarkBlack, justifyContent: 'space-between', paddingRight: darkTheme.padding.base, paddingBottom: darkTheme.padding.xs }}>
						<View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
							<View>
								<RNText style={{ fontFamily: darkTheme.fontFamily.cgMedium, fontSize: darkTheme.fontSize.primaryH2, color: darkTheme.colors.typography }}>{capitalized(loginType === 'studio' ? project_name : name)}</RNText>
								{loginType === 'studio' && (
							...
							{reciever_profile_picture ? <Image source={{ uri: reciever_profile_picture }} resizeMode="cover" style={{ flex: 1, maxHeight: 30, maxWidth: 30, aspectRatio: 1 }} /> : <Person width={30} height={30} />}
						</View>
					</View>
				</Pressable>
			</View>
			<Animated.FlatList
				...
				renderItem={({ item, index }) => <StudioConversationRenderItem last_negotiation_status={last_negotiation_status} last_negotiation_id={last_negotiation_id} party2_id={party2Id} party1_id={party1Id} conversation_id={id} status={status} project_id={project_id} item={item} index={index} data={data} />}
